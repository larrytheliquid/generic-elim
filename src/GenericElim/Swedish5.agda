...
module GenericElim.Swedish5 where
...
record Decl (A : Set) (I : A → Set) (O : (a : A) → I a → Set) : Set₁ where
  field
    {n} : ℕ
    decl : (a : A) → Fin n → Desc (I a) (O a)

  R : (a : A) → Desc

----------------------------------------------------------------------

-- Elim : {A : Set} {I : A → Set} {O : (a : A) → I a → Set}
--   (L : (a : A) → Σ ℕ λ n → Fin n → Desc (I a) (O a))
--   → Set
-- Elim L = (a : A) (n : ℕ)
--   let D = proj₂ (L a)
--       R = σ (Fin n) D
...
--   → Elim L
--   -- (a : A) (n : ℕ)
--   -- (D : Fin n → Desc (I a) (O a))
--   -- → let R = σ (Fin n) D in
--   -- (P : ∀ i → μ R i → Set)
--   -- → Elim R P n D init
-- elim = {!!}
-- elim a n D P = curries n _ _
