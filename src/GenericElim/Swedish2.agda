...
module GenericElim.Swedish2 where
...
branch :  ∀{I O}
  ...
  → Branch R P (D p) (curry φ p)
branch R P ._ D φ here β = β
branch R P ._ D φ (there p) β =
  branch R P _ _ _ p β
...
elim n D P = curries n _ _
  (λ β i x → ind R P (alg R P R init (λ p → branch R P n D init p (β p))) i x)
  where R = σ (Fin n) D
