...
Any type represented as a description can be
introduced with a single introduction rule -- called {\tt init} --
which is the initial algebra for the description.
Any type represented as a description can be
eliminated with a single elimination rule -- called {\tt ind} --
which is the dependently typed version of a catamorphism and as such can be
applied to some algebra.
Eliminating values by defining an appropriate algebra 
can be tedious due to the large amount of low-level detail
description-based datatypes contain. Functions written with {\tt ind} are
verbose but follow a recognizable pattern (\refsec{ind}). 
On the other hand, functions written with standard {\it eliminators}
are shorter and more widely understood.

In this paper we describe how to derive the infinite
class of standard eliminators, as a function -- called {\tt elim} --
...
Ornaments \citep{mcbride2010ornamental} supports the definition of new
description-based datatypes in terms of their relationship with
existing datatypes, and supports the conversion between the two. Free
...
similar in purpose to deriving in {\sc Haskell}, that derives
...
\todo[inline]{mcbride ornament examples}

\acks

We would like to thank Nathan Collins for discussing some of this
