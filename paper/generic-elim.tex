...
({\tt bits}) that computes the decimal value of a binary number, which
is generic in the length of the binary number.

\begin{verbatim}
bits : (n : ℕ) → Vec Bool n → ℕ
bits zero [] = 0
bits (suc n) (false ∷ bs) = bits n bs
bits (suc n) (true ∷ bs) = 2 ^ n + bits n bs
\end{verbatim}
...
partially applying a natural number to {\tt bits}, resulting in a
specialized function that translates bit strings of some constant length
to a decimal value. For example, we specialize {\tt bits} below to
...
bits as a vector of booleans. Any user of {\tt char} needs to know
...
We demonstrate how to create high-level IDSL functions as
generic {\it wrapper} functions on top of existing generic
...
standardizing understanding of this technique
through a series of examples. After presenting the binary to decimal
...
%% For example, a high-level generic wrapper
%% function can be defined in terms of the low-level generic {\tt bits}
%% function. The type of the wrapper will be a {\it function}, containing
%% a stuck computation trying to case-analyze the universe code
%% parameter. However, applying the wrapper function to any
%% particular universe code results allows computation to unfold to some
%% user-friendly type that hides the low-level encoding (defined by the
%% universe's interpretation function).
