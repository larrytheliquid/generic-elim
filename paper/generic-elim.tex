...
\section{Background on Declaring Datatypes}
\label{sec:background}

The goal of this section is to use a first-class encoding of datatype
declarations to define the following type.
...
The goal of this section is to use the primitive introduction rule for
datatypes built using descriptions to define the constructors of
{\tt Vec}.
...
introduce values of type {\tt Vec} using an initial algebra.
...
\begin{verbatim}
consType : (A : Set) (n : ℕ) → Set
consType A n = El (consD A) (Vec A) n

consType A n ⇝ Σ ℕ (λ m → Σ A (λ _ → Vec A m × (suc m ≡ n)))
\end{verbatim}
...
{\tt nilType} or {\tt consType}.

\begin{verbatim}
VecType : (A : Set) (n : ℕ) → Set
VecType A n = El (VecD A) (Vec A) n

VecType A n ⇝ Σ VecT (case (λ _ → Desc ℕ)
  (nilType A n , consType A n , tt))
\end{verbatim}

\subsection{The Type of Endofunctors}

{\tt El} is a a function that computes an endofunctor on families of
indexed sets from a description. The constructor {\tt init}
represents the initial algebra of the functor computed by {\tt El D}.
The initial F-algebra is usually denoted {\tt F X ⟶ X}.
Here, {\tt F = El D} and the arrow is an index-preserving map between
the type families {\tt F X : I → Set} and {\tt X : I → Set}.
...
El : {I : Set} (D : Desc I) → ISet I → ISet I
El (End j) X i = j ≡ i
El (Rec j D) X i = Σ (X j) (λ _ → El D X i)
El (Arg A B) X i = Σ A (λ a → El (B a) X i)
\end{verbatim}

\section{Datatypes from Descriptions}
...
\section{Endofunctors from Descriptions}
...
\section{Examples using Specialized Eliminators}
...
\reffig{ind:concat} and \reffig{ind:append}. These
...
such types consists of 4 parts. The numbers of the parts below
correspond with the numbers in
the code appearing in \reffig{ind:concat} and
\reffig{ind:append}.

\begin{myparte}[Eliminating with {\tt ind}]
\label{parte:one}
