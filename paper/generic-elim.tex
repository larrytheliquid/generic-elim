...
\title{Deriving Type Theory from Descriptions}
\subtitle{User-Friendly Dependently Typed Generic Wrapper Functions}
...
wrapper function whose type for an arbitrary value is some non-uniform
stuck computation.
...
axioms of an {\tt open} language. Similarly, the type of {\tt inj}
applied to a {\tt Data}, and a tag specifying a constructor, is exactly the
expected type of the introduction axiom in an
{\it open} language.
...
non-uniform wrapper function. To this
end, we highlight each {\bf Part$_I$} involved in defining the
...
end, we highlight each {\bf Part$_E$} involved in defining the non-uniform
