...
indCurried D P α i x =
  ind D P (uncurryHyps D (μ D) P init α) i x
\end{verbatim}
...
instead only have ``one constructor'' with arguments.

\subsection{Uncurried Eliminator}

\todo[inline]{dont read this yet; work in progress}
In order to implement \refparte{one}, we need our eliminator to expect a
description built as sum (constructors) of products (arguments), and
implicitly perform case analysis over this sum. We specialize the
elimination principle to sums of products by parameterizing not by a description,
but by an {\tt E : Enum} and a function {\tt C} from tags of that enumeration to
descriptions representing the constructor choices. Recall from
