...
generic functions of the metalanguage {\sc Agda}. They are a representative
...
The universe of codes for bit strings of some length is the
type of natural numbers ({\tt ℕ}), and the interpretation function is
a vector of booleans ({\tt Vec Bool}) of some length code.
We can get an instance of this generic program by
partially applying a natural number to {\tt decimal}, resulting in a
specialized function that translates bit strings of some constant length
to a decimal value. For example, we specialize {\tt decimal} below to
{\tt 8}, giving us a binary to decimal converter for unsigned
characters.
...
Writing generic functions in this universe-based style is pleasant,
but we may wish to expose a more user-friendly generic
function in the form of an
internal domain-specific language (IDSL)~\cite{TODO}. In some sense,
the specialized type of {\tt char} reveals the low-level encoding of
decimal as a vector of booleans. Any user of {\tt char} needs to know
...
When writing an IDSL, we may want to instead expose a version of
{\tt char} that lets us write following high-level expression instead.

\begin{verbatim}
fourty-two : ℕ
fourty-two = char 0 0 1 0 1 0 1 0
\end{verbatim}

We demonstrate how to create high-level IDSL functions by using
generic {\it wrapper} functions, which wrap existing generic
universe-based functions. This technique is folklorish, and almost
always involves some form of currying values in the universe. However,
wrapper functions may also perform other kinds of transformations to
translate between low-level and high-level data (e.g. coercing numbers
to booleans in the example above).

{\bf Our contribution} is
standardizing understanding of writing IDSL's via generic wrapper
functions through a series of examples. After presenting the binary to decimal
converter in more detail, we show how to internalize type formers,
constructors, and eliminators of an open
\citet{martin1975intuitionistic} type theory in terms of the
description-based closed type theory primitives of
\citet{Chapman:2010:GAL:1932681.1863547}. Each of these open type
theory constructions can be defined as a generic wrapper function
(making open type theory an IDSL) in
terms of the existing generic functions modelling closed type
theory primitives. Of these constructions, the generic eliminator is
the most complex, requiring the definition of several helper wrapper
generic functions and performing several data transformations besides
currying.
...
\begin{itemize}
\item{\bf{\refsec{bits}}}
{\it Formalizes} the technique of writing generic wrapper functions.
The binary to decimal converter is used as a motivating example.

\item{\bf{\refsec{background}}}
{\it Reviews} how to define datatypes
using descriptions within an {\sc Agda} model the closed type theory of
\citet{Chapman:2010:GAL:1932681.1863547}.
...
\section{Generic Binary to Decimal Converter}
\label{sec:bits}

The goal of this section is to learn how to define generic wrapper
functions, which may be used to define IDSL functions like
{\tt bits}. The function {\tt bits} is an IDSL for creating
user-friendly converters from a binary string of some length to a
decimal value. It can be defined in terms of a generic wrapper over
the low-level binary to decimal converter {\tt decimal}, and can be
used as follows.
...
At the end of this section we formalize the interface to defining
generic wrapper functions as the dependent record {\tt Wrapper}.

\subsection{Directions}

Recall that our universe of codes is the natural numbers ({\tt ℕ}),
and our interpr
