...
\title{Generic Constructors \& Eliminators from Descriptions}
...
VecType A n ⇝ Σ VecT (case (λ _ → Set)
  (nilType A n , consType A n , tt))
\end{verbatim}
\todo[inline]{Motive for case is wrong}
...
{\tt VecType}, the type of the argument to {\tt init} for vectors.
...
In order to prove that some property {\tt P} holds for any value of a
described type, you must supply an algebra argument {\tt α} showing
that {\tt P} holds for any {\tt El D (μ D) i}. Whereas an eliminator
...
In order to prove {\tt P} you get the following
arguments:
